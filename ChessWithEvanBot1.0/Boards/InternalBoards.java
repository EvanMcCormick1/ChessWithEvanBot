...
public class InternalBoards {
    ...
    private Stack<Move> movesPlayedList = new Stack<>();
    private Stack<Move> futureMovesList = new Stack<>();
    ...
    // SET FUNCTIONS
    public void setMovesPlayedList(Stack<Move> movesPlayedList) {
    ...
    public void demotePiece(int pos, ChessPiece np) {
        chessPieceBoard[pos] = capturedPieceStack.pop();
        integerBoard[pos] = chessPieceBoard[pos].getType();
    }
    ...
    public boolean executeMove(int s, int f) {
        if (isLegalMove(s, f)) {
            Move move = chessPieceBoard[s].generateMove(f, integerBoard, chessPieceBoard, movesPlayedList,
                    checkChecker);
            executeMove(move);
            return true;
        } else
    ...
    public int getBoardEval() {
        return boardEvaluator.evaluate(integerBoard);
    }
    ...
    public ArrayList<Move> allPossibleMovesOrdered() {
        ...
        for (int i = 0; i < 64; i++) {
            ...
            if (integerBoard[j] > 0 && ((integerBoard[j] - 1) / 6 == getActivePlayer())) {
                for (Move move : chessPieceBoard[j].generateLegalMoves(integerBoard, chessPieceBoard, checkChecker,
                        movesPlayedList)) {
                    if (move.isCheck())
                        checks.add(move);
    ...
    public ArrayList<Move> allCaptures() {
        ArrayList<Move> moves = new ArrayList<Move>();
        for (int i = 0; i < 64; i++) {
            if (integerBoard[i] > 0 && ((integerBoard[i] - 1) / 6 == getActivePlayer())) {
                for (Move move : chessPieceBoard[i].generateCaptures(integerBoard, chessPieceBoard, checkChecker,
                        movesPlayedList)) {
                    moves.add(move);
                }
            }
    ...
    public ArrayList<Move> allChecksCapturesOrdered() {
        ArrayList<Move> checks = new ArrayList<>();
        ArrayList<Move> captures = new ArrayList<>();

        for (int i = 0; i < 64; i++) {
            if (integerBoard[i] > 0 && ((integerBoard[i] - 1) / 6 == getActivePlayer())) {
                for (Move move : chessPieceBoard[i].generateCheckCaptures(integerBoard, chessPieceBoard, checkChecker,
                        movesPlayedList)) {
                    if (move.isCheck())
                        checks.add(move);
                    else if (move.isCapture())
    ...
    public void makeRandomMove() {
        ArrayList<Move> moves = allPossibleMovesOrdered();
    ...
    public int miniMax(int depth, Move move) {
    ...
    public void alphaBetaTreeSearch(int depth) {
        ...
        if (getActivePlayer() == 1) { // Maxi (White wants highest score)
            ...
            if (moves.size() > 0) {
                for (int i = 0; i < moves.size(); i++) {
                    int eval = alphaBetaMiniMax(depth - 1, alpha, beta, moves.get(i));
                    System.out.println("Move # " + i + " Eval: " + eval);
                    if (eval < bestEval) {
                        bestEval = eval;
                        beta = eval;
                        index = i;
    ...
    public int alphaBetaMiniMax(int depth, int alpha, int beta, Move move) {
    ...
    public int alphaBetaChecksCaptures(int depth, int alpha, int beta, Move move) {
        ...
        if (depth <= 0) {
            ...
            if (getActivePlayer() == 1) { // Maxi (White wants highest score)
                ...
                ArrayList<Move> moves = allChecksCapturesOrdered();
                if (moves.size() > 0) {
                    for (int i = 0; i < moves.size(); i++) {
    ...
    public int alphaBetaCaptures(int depth, int alpha, int beta, Move move) {
        ...
        if (getActivePlayer() == 1) { // White to Play.
            ...
            ArrayList<Move> moves = allCaptures();
            if (moves.size() > 0) {
                for (int i = 0; i < moves.size(); i++) {
